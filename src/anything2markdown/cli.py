...
def run(input_dir: Path | None, output_dir: Path | None, verbose: bool, strategy: str | None, mirror: bool):
    ...
    if input_dir:
        ...
        if not output_dir and not mirror:
            settings.output_dir = input_dir / (input_dir.name + "AI工作专区")
    if output_dir:
...
def parse_file(file_path: Path, output_dir: Path | None, strategy: str | None):
    """Parse a single file."""
    # Setup
    setup_logging()
    ...
    try:
        ...
        if (
            ...
            click.echo("MarkItDown failed on this PDF, trying OCR fallback...")
...
def init():
    ...
    click.echo("Initialized Anything2Markdown directories:")
