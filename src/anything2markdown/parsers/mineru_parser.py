...
class MinerUParser(BaseParser):
    ...
    def is_available(self) -> bool:
        """MinerU needs a valid API key to be routable."""
    ...
    def parse(self, file_path: Path, output_dir: Path) -> ParseResult:
        ...
        try:
            ...
            pages_per_split_limit = settings.mineru_pages_per_split
            needs_split = page_count > pages_per_split_limit or file_size_mb > MAX_FILE_SIZE_MB
            ...
            if needs_split:
                ...
                if file_size_mb > MAX_FILE_SIZE_MB:
                    ...
                    pages_per_split = max(50, min(pages_per_split, pages_per_split_limit))
                else:
                    pages_per_split = pages_per_split_limit
    ...
    def _get_page_count(self, file_path: Path) -> int:
        ...
        try:
            ...
            logger.warning("pypdf not installed, assuming large PDF")
            # If PyPDF2 not available, estimate based on file size
            # Rough estimate: 100KB per page
            size_kb = file_path.stat().st_size / 1024
            return max(1, int(size_kb / 100))

    def _split_pdf(self, file_path: Path, pages_per_split: int = 400) -> list[Path]:
        """
    ...
    def _process_large_pdf(
        self, file_path: Path, page_count: int, pages_per_split: int = 400
    ) -> str:
        """
